...
public class CRequisicao {
	...
	public int insert(String idService, String resp) throws JAXBException {
		...
		RequestBody formBody = new FormBody.Builder().add("idService", idService).add("response", resp).build();
		Request request = new Request.Builder().url("http://localhost:8080/Service_Health/ws/servico/requisicao/insert")
				.post(formBody).build();
		try {
	...
	public List<Requisicao> getList() throws JAXBException {
		...
		Request request = new Request.Builder()
				.url("http://localhost:8080/Service_Health/ws/servico/requisicao/getList").get().build();
	...
	public List<LastRequest> getLastOnes() throws JAXBException {
		...
		RequestBody formBody = new FormBody.Builder().build();
		Request request = new Request.Builder()
				.url("http://localhost:8080/Service_Health/ws/servico/requisicao/getLastOnes").get().build();
	...
	public EstatisticasServicoDia getDay(String id, String day) throws JAXBException {
		OkHttpClient client = new OkHttpClient();

		int code = 0;

		System.out.println(day);
		RequestBody formBody = new FormBody.Builder().add("id", id).add("day", day).build();
		Request request = new Request.Builder().url("http://localhost:8080/Service_Health/ws/servico/requisicao/getDay")
				.post(formBody).build();
		...
		try {
			...
			JAXBContext jaxbContext = JAXBContext.newInstance(EstatisticasServicoDia.class);
			Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
			StringReader reader = new StringReader(xmlString);

			estatisticasDoDiaDeUmServico = (EstatisticasServicoDia) unmarshaller.unmarshal(reader);
